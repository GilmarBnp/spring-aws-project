...
class BookServiceTests {
	...
	void testFindById() throws Exception {
		Book book = input.mockEntity((double) 2);
		...
		System.out.println("RESULT BOOK NUMBER 2 LAUNCHDATE"+ result.getLaunchDate());
		
		assertEquals("Title2.0", result.getTitle());
		assertEquals("Author2.0", result.getAuthor());
	...
	void testCreate() throws Exception {
		Book entity = input.mockEntity((double) 2);
		Book persisted = entity;
		entity.setId(1L);
		
		BookVO vo = input.mockVO((double) 2);
		vo.setId(1L);
	...
	void testFindAll() throws Exception {
		...
		PagedModel<EntityModel<BookVO>> bookPage = (PagedModel<EntityModel<BookVO>>) service.findAll(pageable);
		
		for (EntityModel<BookVO> bookEntity : bookPage) {
		    BookVO book = bookEntity.getContent(); 
		    if (book != null) {
		        System.out.println(book.getAuthor() + " " + book.getAuthor());
		    }
		}
		
	    List<EntityModel<BookVO>> bookPageList = new ArrayList<>(bookPage.getContent());

		
		if (bookPageList.size() > 0) {
			System.out.println(bookPageList.get(0).getContent().author + bookPageList.get(0).getContent().title);
			
			assertNotNull(bookPageList);
			assertEquals(14, bookPageList.size());
			...
			assertTrue(bookTwo.toString().contains("links: [<http://localhost/api/book/2>;rel=\"self\"]"));	
		
			assertEquals("Title2.0", bookTwo.getTitle());
			assertEquals("Author2.0", bookTwo.getAuthor());
			...
			System.out.println("FINDALL BOOK 8 TITLE " + bookPageList.get(8).getContent().getTitle());
			
			assertNotNull(bookEight);
			assertNotNull(bookEight.getId());
			assertNotNull(bookEight.getLinks());
			
			assertTrue(bookEight.toString().contains("links: [<http://localhost/api/book/8>;rel=\"self\"]"));	
			
			assertEquals("Title8.0", bookEight.getTitle());
			assertEquals("Author8.0", bookEight.getAuthor());
			...
			assertNotNull(bookEleven.getId());
			assertNotNull(bookEleven.getLinks());
			
	        assertTrue(bookEleven.toString().contains("links: [<http://localhost/api/book/11>;rel=\"self\"]"));	
			
	    	assertEquals("Title11.0", bookEleven.getTitle());
			assertEquals("Author11.0", bookEleven.getAuthor());
			assertEquals(11.0, bookEleven.getPrice());
